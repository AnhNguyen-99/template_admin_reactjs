...
import { register_customer } from 'services/AuthService';
...
const FormRegister = ({ ...others }) => {
    ...
    const submit = (username, email, password) => {
        register_customer(username, email, password)
            .then(response => {
                showNotification('Register customer success', 'success');
                console.log(response.customer);
                window.location.replace("/login")
            }).catch(error => {
                console.log(error)
                showNotification(error?.detail !== null ? error.detail : 'Register customer fail', 'danger');
            });
    }
